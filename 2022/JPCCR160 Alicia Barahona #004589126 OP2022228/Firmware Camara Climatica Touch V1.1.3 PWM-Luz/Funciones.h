...
void LeeEEPROM(){
   ...
   Segmento=EEPROM_ReadByte(15);delay_ms(5);  
   if(Segmento>24){
      controlT=0;
      controlH=0;
      controlR=0;
...
void LimitaValores(){
   ...
   if(calib[1].patron1>30 || calib[1].patron1<1){
      calib[1].patron1=10;write_eeprom(76,10);delay_ms(20);write_eeprom(77,0);delay_ms(20);
   }
   if(calib[1].patron2>70 || calib[1].patron2<30){
      calib[1].patron2=50;write_eeprom(78,50);delay_ms(20);write_eeprom(79,0);delay_ms(20);
   }
   if(calib[1].patron3>100 || calib[1].patron3<70){
      calib[1].patron3=70;write_eeprom(80,70);delay_ms(20);write_eeprom(81,0);delay_ms(20);
   }
   if(calib[1].control1>30 || calib[1].control1<1){
      calib[1].control1=10;write_eeprom(82,10);delay_ms(20);write_eeprom(83,0);delay_ms(20);
   }
   if(calib[1].control2>70 || calib[1].control2<30){
      calib[1].control2=50;write_eeprom(84,50);delay_ms(20);write_eeprom(85,0);delay_ms(20);
   }  
   if(calib[1].control3>100 || calib[1].control3<70){
      calib[1].control3=70;write_eeprom(86,70);delay_ms(20);write_eeprom(87,0);delay_ms(20);
   }  
    ...
    if(flagRampa){
      if(Segmento==0){
         if(flag_calcula==0){
            ...
            Ganancia=GananciaR*(Setpoint/20);
            entroControl=OFF;
...
// Pendiente determinar si el comportamiento de la mufla es lineal 10°C/min o si tiene un comportamiento diferente.         
      ...
      if(Enfria==1){//Control Enfriamiento
         ...
         if(error<(-desvio-0.2) && Aumento>-0.2){
            if(Potencia<90.0){
...
float leeHumedad(int8 media3){
   ...
   float PromH,PromTa,Tcalculada=0.0;
   float Href;
   PromH=0.0;
   PromTa=0.0;
   ...
   Tam=PromTa;
   Tcalculada=(PromTa+Temperatura)/2.0;
   //RH =((-0.0000000019206*pow(VH,3))+( 0.000014370*pow(VH,2))+( 0.0034210*VH)-12.4)/(1+(PromTa-23)*( 0.0024));//Ecuacion Original
   RH =((-0.0000000019206*pow(VH,3))+( 0.000014370*pow(VH,2))+( 0.0034210*VH)-12.4)/(1+(Tcalculada-23)*( 0.0024));
   if(RH<0)
        RH=0.0;
   //if(RH>100.0)
        //RH=100.0;
   ...
   PromH=PromH/media3;
    ...
    if(PromH<=calib[1].control2){
        Href=(PromH-calib[1].b1)/calib[1].m1;
    }else{
        Href=(PromH-calib[1].b2)/calib[1].m2;
    }
   
   if(calculaMediana){
      calculaMediana=OFF;
      //medianaHumedad[yy]=PromH;
      medianaHumedad[yy]=Href;
      yy++;
      if(yy>=2){
         yy=0;
...
void controlaHumedad2(){
   if(Humidificar){
         ...
         if(Humedad>SetpointH+3.0){
            ...
            if(!Infinito)   
               ...
               flagHumedad=OFF;
         }else if(Humedad<SetpointH-0.1 && Humedad>SetpointH-3.0){
            dehumidificador=OFF;
            tdehumidificador=0;
            CR2_Write(0);//Humidificador_off;
            CR1_Write(0);//Dehumidificador_off;
   ...
   if(alarmaH){
      if(medianaH<(SetpointH-alarmaHumedad)){
...
void Carga_Vector(void){
   ...
   SlaveB0Tx[22]=MAC[4];
   SlaveB0Tx[23]=MAC[5];
   SlaveB0Tx[24]=(int8)Tam;
   SlaveB0Tx[25]=extraeDecimal(Tam);
}
