...
short desconectoTemperatura=OFF,desconectoHumedad=OFF,graficaa=OFF,flagMotor=OFF,Humidificar=0,dehumidificador=0,humidificador=0,flagHumedad=0,flagAlarma=0,flagBuzzer=0,flagFinaliza=0;
...
int8 T1Humedad=0,T2Humedad=0,T3Humedad=0,tDisplay=0,conteoAlarma=0,ajustaIP=0;
...
int8 Vel1=0,SetpointH=0,alarma=0,nivelBajo=0,nivelAlto=0,bomba=0,yy=0;  
...
float Setpoint=0,SetpointAnt=0,minH=0,maxH=0,alarmaTemperatura=0,alarmaHumedad=0,d6=0,tmp3=0,PotenciaH=0;
int32 tGrafica=0;
...
float Ganancia=0,errorH=0,TpulsoH=0.0,TcicloH=0.0,GananciaH=0,GananciaRH=0,dewPoint=0.0;
...
short Finalizo=0,conteoPregunta=0,imprime=0,Infinito=0,flagControlT=0,flagControlH=0,flag_clear2=0,flag_ganancia=0,flag_clear=0,entroControl=OFF,entroControlH=OFF;
float X[2]={0.0,0.0};
...
float Prom=0,Prom2=0,V0=0,V1=0,Temperature=0,TemperaturaPt1002=0.0,Pt=0.0,Pt2=0.0,TiempoRampa=0,Tam=0,RH2=0,medianaH=0.0;
...
void temp1s(void){
   ...
   if(controlaH==1){
      if(tiempoctrl<TpH){ // Si el tiempo es menor o igual al tiempo de control se enciende la resistencia de lo contrario se apaga.
         DC2_on;//Humidificador_on;
         DC3_on;//Ventilador Humidificador
      }else{
         DC2_off;//Humidificador_off;
   ...
   if(flagAlarma)
    ...
    if(tiempos>=200){  // Cuenta cada 5ms   
        ...
        if(Menu==20 && !entroControl && flagControlT){
            tCalentamiento++;
         }else{
            tCalentamiento=0;
         }
         
         if(Menu==20 && !entroControlH && flagControlH){
            thumidificacion++;
         }else{
            thumidificacion=0;
         }
...
{ 
    ...
    n=0;
    for(i=0;i<24;i++){
      rampas[i].TemperaturaInt=read_ext_eeprom((i*11));
      rampas[i].TemperaturaDec=read_ext_eeprom(1+(i*11));
      rampas[i].Humedad=read_ext_eeprom(2+(i*11));
      rampas[i].Horas=read_ext_eeprom(3+(i*11));
      rampas[i].Minutos=read_ext_eeprom(4+(i*11));
                    
      rampas[i].Temperatura=(inttofloat(rampas[i].TemperaturaDec)+(float)rampas[i].TemperaturaInt);
      if(rampas[i].Temperatura>60)rampas[i].Temperatura=10;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
    if(Menu==20){ //Menu de Funcionamiento
      ...
      conteoPregunta=OFF;
      tPregunta=0;
      dewPoint=Temperatura+(35.0*log10(Humedad/100.0));
      if(flagRampa){
        ...
        printf("titulo.txt=\"Camara Climatica JP Inglobal Pr:%2.1f°C\"",dewPoint);
      }
      SendDataDisplay();
      printf("fecha.txt=\"Fecha:%02u/%02u/20%02u    Hora: %02u:%02u:%02u\"",Dia,Mes,Year,Hora,Minuto,Segundo);
      SendDataDisplay();
      ...
      if(flagHumedad && DOOR){         
         if(medianaH>(SetpointH+alarmaHumedad) || medianaH<(SetpointH-alarmaHumedad)){
            alarmaH=ON;
         }else{
            alarmaH=OFF;
            tHumidificacion=0;
      ...
      if(Rx_Buffer[4]==0x0a){//Calibraacion
         printf("page Clave");
         SendDataDisplay();
         TipoClave=5;
      }
      ...
      if(alarma==10){//Alarma por que no lleno el reservorio del humidificador
         flagControlH=0;
      }
      
      if(flagControlT){         
         if(inicioControlH && !Humidificar){
            flagControlH=ON;
         }else if(entroControl && inicioControlH){
            flagControlH=ON;
         }else{
            flagControlH=OFF;
         }
      ...
      if(Finalizo==0){
         ...
         if(!flagFinaliza){
            EEPROM_WriteByte((int8)flagControlH,42);delay_ms(20);
            EEPROM_WriteByte((int8)flagControlT,41);delay_ms(20);
            EEPROM_WriteByte(0,43);delay_ms(20);
            write_eeprom(15,0);delay_ms(20);
            flagFinaliza=1;
         }
      }
         ...
         if(flagRampa){
            if((mEst>=mEstp)&&(hEst>=hEstp)&& Finalizo==0){
               ...
               if(Segmento<Segmentos-1){
                  Segmento++;
                  write_eeprom(15,Segmento);delay_ms(20);
                  hEst=0;
                  mEst=0;
                  entroControl=OFF;
                  tCalentamiento=0;
