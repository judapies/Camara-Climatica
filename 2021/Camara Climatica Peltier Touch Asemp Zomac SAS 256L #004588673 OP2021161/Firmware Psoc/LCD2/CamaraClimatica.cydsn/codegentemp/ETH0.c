...
const uint16 ETH0_SOCKET_TX_BASE[4] = { 0x4000, 0x4800, 0x5000, 0x5800 };
const uint16 ETH0_SOCKET_RX_BASE[4] = { 0x6000, 0x6800, 0x7000, 0x7800 };

static ETH0_SOCKET ETH0_SocketConfig[4];
static uint32 ETH0_SubnetMask;

static uint8 ETH0_MAC[6]; /* V1.2: removed = {0x00, 0xDE, 0xAD, 0xBE, 0xEF, 0x00}; */
...
#define ETH0_SpiDone    ((SPI0_SpiUartGetTxBufferSize()==0)?1:0)
/*
#if (0 == 0)
...
ETH0_Start( void )
{
	...
	if ( ETH0_ParseMAC("0x00, 0xDE, 0xAD, 0xBE, 0xEF, 0x00", &ETH0_MAC[0]) == CYRET_BAD_DATA) {
		ETH0_MAC[0] = 0;
		ETH0_MAC[1] = 0xDE;
		ETH0_MAC[2] = 0xAD;
		ETH0_MAC[3] = 0xC0;
	...
	/* END of V1.2 Update ------ */
	ip = ETH0_ParseIP("192.168.0.80");
	sub = ETH0_ParseIP("255.255.255.0");
	gateway = ETH0_ParseIP("192.168.0.1");
